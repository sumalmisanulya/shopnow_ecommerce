...
export default function AdminInvoicesPage() {
  ...
  const getInvoiceStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "CANCELLED":
        return "bg-rose-500/10 text-rose-450 border border-rose-550/20";
      default:
        return "bg-zinc-550/10 text-zinc-400 border border-zinc-550/20";
    }
  };

  const getDownloadUrl = (order: Order) => {
    // Extract customer name if we can parse it from COD format: "Name, Address, City, ZIP 12345"
    // Our COD address format is: shippingName is saved inside shippingAddress/phone but let's check
    // Wait, in cart/page.tsx:
    // finalReceiver = paymentMethod === "cod" ? shippingName : (session.user?.name || "Customer")
    // Wait, finalReceiver is encoded in success url but in order:
    // shippingAddress is fullAddress (shippingAddress, shippingCity, ZIP shippingZip)
    // So the receiver name is NOT in shippingAddress directly, but we can look for it.
    // Let's pass the logged in user name or check if there is an alternative
    const name = session?.user?.name || "Customer";
    const email = session?.user?.email || "customer@shopnow.com";
    ...
    return `/api/invoices/download?${params.toString()}`;
  };
