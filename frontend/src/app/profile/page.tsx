        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="glass p-6 rounded-2xl border border-white/5 text-left">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h3 className="font-bold text-zinc-200">{session?.user?.name || "Premium User"}</h3>
                <span className="inline-block px-2 py-0.5 rounded-full text-[10px] font-bold bg-violet-600/10 text-violet-300 mt-1 uppercase border border-violet-500/10">
            ...
            <form onSubmit={handleReturnSubmit} className="space-y-4">
              <div>
                ...
                >
                  ...
                  {orders
                    .filter((ord) => ord.status === "PENDING" || ord.status === "CONFIRMED")
                    .map((ord) => (
              ...
              <div className="space-y-6">
                {orders.map((ord) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mt-5 pt-3.5 border-t border-white/5">
                      <p className="text-sm font-bold text-zinc-200">
                        Total Price: <span className="text-violet-400">${ord.totalPrice.toFixed(2)}</span>
                      </p>
                      
                      <div className="flex gap-2 w-full sm:w-auto">
                        <a
                          href={`/api/invoices/download?orderCode=${ord.code}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-xs font-semibold text-zinc-200 transition-colors w-full sm:w-auto"
                        >
