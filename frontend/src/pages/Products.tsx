  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        const res = await fetch(`${backendUrl}/api/products?${searchParams.toString()}`);
        if (res.ok) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <Link to="/" className="inline-flex items-center text-xs font-semibold text-zinc-500 hover:text-zinc-300 transition-colors mb-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <main className="lg:col-span-3">
          {loading ? (
            <div className="glass rounded-2xl border border-white/5 p-24 flex items-center justify-center">
